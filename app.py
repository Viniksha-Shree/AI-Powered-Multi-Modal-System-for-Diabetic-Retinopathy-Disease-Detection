...
def inference_both(model, tokenizer, text, image_file, transform, device, id2label, max_length=128, return_logits=False):
    cleaned_text = allow_alphabets_and_punctuation(text)
    encoding = tokenizer(cleaned_text, padding="max_length", truncation=True,
                         max_length=max_length, return_tensors="pt")
    for key in encoding:
        encoding[key] = encoding[key].to(device)
    image = Image.open(image_file).convert("RGB")
...
# -----------------------------------------------------------------------------
def allow_alphabets_and_punctuation(text):
    # Keep only alphabets, punctuation, and whitespace
    ...
    if mode == "Text Only":
        with st.container():
            ...
            with col1:
                ...
                st.session_state.text_input = st.text_area("Enter Text Input",value=st.session_state.text_input, placeholder="Enter your text here...")
            with col2:
                st.selectbox("Sample Text", sample_texts)
            final_text = st.session_state.text_input
            cleaned_text = allow_alphabets_and_punctuation(final_text)
            if st.button("Predict (Text)"):
                if final_text.strip() == "":
                    st.error("InputError: Please provide valid text input.")
                elif len(cleaned_text.strip()) == 0:
                    st.error("ValueError: Text must contain only letters and punctuation (no numbers or symbols.)")
                elif len(final_text.strip().split()) < 10:
                    st.warning("Insufficient data: Please enter at least 10 words for better predictions.")
                else:
        ...
        with st.container():
            ...
            if "text_input" not in st.session_state:
                st.session_state.text_input = ""
            st.session_state.text_input = st.text_area("Enter Text Input",value=st.session_state.text_input, placeholder="Enter your text here...", key="both_text")
            cleaned_text = allow_alphabets_and_punctuation(st.session_state.text_input)
            ...
            if st.button("Predict (Both)"):
                if uploaded_file is None:
                    ...
                    st.error("ValueError: Text must contain only letters and punctuation (no numbers or symbols.)")
                elif len(st.session_state.text_input.strip().split()) < 10:
                    st.warning("Insufficient data: Please enter at least 10 words for better predictions.")
                else:
                    with st.spinner("Processing combined input..."):
                        if show_details:
                            pred_label, logits = inference_both(model, tokenizer, st.session_state.text_input, uploaded_file, image_transforms, device, id2label, return_logits=True)
                        else:
                            pred_label = inference_both(model, tokenizer, st.session_state.text_input, uploaded_file, image_transforms, device, id2label)
                        time.sleep(1)
                        st.success(f"Predicted Label: **{pred_label}**")
                        input_context = f"Text: {st.session_state.text_input[:30]}..., Image: {uploaded_file.name}"
